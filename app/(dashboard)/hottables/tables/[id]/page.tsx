...
import { selectDerivedLineItems } from '@/store/services/aiPdf/selectors'
...
const isPersistedTableId = (tableId?: string) => Boolean(tableId && !tableId.startsWith('merged-'))


const formatSyncSummary = (summary: FrontendSyncSummary) => (
    `Tables +${summary.createdTables} / ~${summary.updatedTables} / -${summary.deletedTables}, ` +
    `Rows +${summary.createdRows} / ~${summary.updatedRows} / -${summary.deletedRows}`
)
...
function PdfTableGrid({
    ...
    onTableColumnsChange: (tableId: string, columns: AiPdfTable['columns']) => void
    ...
    const applyColumnMappings = () => {
        ...
        onTableColumnsChange(table.id, updatedColumns)
        ...
        <div className={index > 0 ? 'border-t-2 border-slate-200' : undefined}>
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-5 py-3.5">
                ...
                <div className="flex items-center gap-2">
                    ...
                    onClick={openUpdateColumnsModal}>
                        <EditOutlined />                      
                    </Button>
            ...
            <Modal open={updateColumnsOpen} footer={null} onCancel={() => setUpdateColumnsOpen(false)} width={880}>
                ...
                <div style={{ display: 'grid', gap: 12 }}>
                    {columnMappings.map((mappingRow, rowIndex) => {
                        ...
                        return (
                            <div key={mappingRow.sourceColumnKey || rowIndex} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'center' }}>
                                <Select
                                    allowClear
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 24 }}>
                    ...
                    <Button type="primary" onClick={applyColumnMappings}>
                        Apply
                    </Button>
                </div>
            </Modal>
...
export default function UploadDetailPage() {
    ...
    const lineItems = useAppSelector(useMemo(() => selectDerivedLineItems(uploadId), [uploadId]))
    ...
    const handleTableColumnsChange = (tableId: string, columns: AiPdfTable['columns']) => {
        setTables((prev) =>
            prev.map((table) => (table.id === tableId ? { ...table, columns } : table))
        )
    }
        ...
        <div className="min-h-screen bg-[#f8f9fc] p-8">
            <div className="mb-7 flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Tag className="rounded-md border-none bg-emerald-50 font-semibold text-emerald-600">
                        {lineItems.length} derived {lineItems.length === 1 ? 'line item' : 'line items'}
                    </Tag>
                </div>
            </div>
